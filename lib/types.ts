 ...
 */
export type Form = "dry" | "wet" | "litter" | "treat";
...
export interface Merchant {
   ...
   *
   * 跟 dead 是兩回事，混在一起會出事：
   *   dead 是「這條連結不能用了」，下架、關店、連結過期，基本上不會回來
   *   soldOut 是「東西賣完了」，補貨就回來，連結本身完全正常
   *
   * 2026-09-20 之前沒有這個欄位，缺貨的只能標成失效或乾脆不收，
   * 結果是 Tim 產好的連結被丟掉，補貨之後要重產一次。
   *
   * 讀者看不到 soldOut 的賣場（點進去是缺貨頁，那比沒有連結更糟），
   * 但資料留著，維護台會列出來提醒回去看補貨了沒。
   */
  soldOut?: boolean;
  /**
...
export interface Product {
  ...
  /**
   * 找過、但蝦皮找不到的紀錄（只給維護頁看）。
   * 有這一欄的，待補清單排到最後，並寫出原因，Tim 不用再白找一次。
