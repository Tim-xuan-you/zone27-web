...
export type TableCall = {
  ...
  /** 對戰(客 vs 主 · 不含運彩編號) */
  match: string;
  /** 賽事開打時間(台北 · 例「6/22 09:35」)· 讓人知道是哪一場 + 何時開打 · Tim 從運彩單轉述時一起給。
      🔴 誠實頁鐵律:不知道就留空(graceful 不顯)· 絕不捏造時間。 */
  when?: string;
...
// 🔴 R241 國小生化(Tim dogfood「全部寫得很奇怪、不懂」):call 一律白話(費城人會贏 /
//    兩隊總分不到 7.5 · 不用「不讓分/大小盤」運彩術語)· resultNote 用「中了/沒中」·
...
const TABLE: TableCall[] = [
  ...
  // ── 2026-06-20 起這一桌 ──────────────────────────────
  {
    ...
    resultNote: "角球 8 顆(日本 5 + 突尼西亞 3)· 不到 8.5 → 沒中",
    result: "lose",
  },
  {
    handle: "Ron",
  ...
  },
  {
    ...
    call: "運動家 會贏",
    engineModels: true,
    resultNote: "天使 7:0 運動家 · 運動家 0 分 → 沒中",
    result: "lose",
  },
  {
    handle: "Tim",
    ...
    call: "兩隊總分 超過 8.5 分",
    engineModels: true,
    resultNote: "兩隊合計 14 分 · 超過 8.5 → 中了",
    result: "win",
  },
  {
    handle: "Ron",
  ...
  },
  {
    ...
    engineModels: false,
    engineNote: "角球這種賭法,我們引擎沒在算 —— 還是照樣幫他對帳",
    resultNote: "角球 8 顆(海地 4 + 巴西 4)· 不到 9.5 → 沒中",
    result: "lose",
  },
  {
    handle: "Tim",
