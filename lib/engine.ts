...
function score(p: Product, situation: Situation): number {
  ...
  /*
   * Omega-3 以前不管使用者問什麼都加最多 12 分。那是在獎勵「有把數字印出來」的品牌。
   *
   * 2026-09-18 抓到：問「成貓」，答案從第一饗宴海魚（每公斤 386）變成紐頓 T24（每公斤 602），
   * 差別只在紐頓公布 omega-3 1.2%、第一饗宴沒公布（當成 0.5）。
   * 一隻沒有皮膚問題的貓，不需要為了這個多付兩百塊。
   *
   * 皮膚、毛髮的情況上面已經加過分了（omega-3 對皮毛是有依據的方向），這裡不再重複加。
   */
