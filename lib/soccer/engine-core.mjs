...
// ── 賭徒熟悉的足球玩法視角(大小球 / 讓球 / 兩隊進球)· 從同一張比分表推導 ─────────────
// 台灣運彩看足球,腦袋裝的是「大小球 2.5 大還小?」「讓 1.5 球守不守得住?」——不是 xG。
// 棒球側已上「讓分 + 大小盤」(deriveRunLineProbs / deriveTotalLineProbs · 從同一份模擬推)。
// 這是它的足球雙生:把「算出勝/平/負」那張 Dixon-Coles 比分機率表,換賭徒熟悉的角度再讀一次。
//
// 🔴 紅線:純機率陳述(.5 線 → 無和局 → 互補加總 100)· 不接莊家盤口、不轉賠率、不出「押這邊」。
//   重建用同一份 DEFAULTS(rho / maxGoals),且吃的是預測自帶的 λ(xgHome/xgAway)→ 跟卡片上
//   秀的 勝/平/負 % 同一張表、同源(零 drift)· 數字我們自己算,你自己判斷。
//
// 註(不變式):這三個玩法「不另存進 soccer-locked.json」· 而是每次 render 從鎖定預測的 λ
//   即時重算。 對已鎖定的場,卡片預測由 predictFromGoals(locked.xgHome, locked.xgAway) 重現
//   (見 football-data.ts),所以本函式吃到的就是鎖定的 λ → 玩法機率自動繼承那把鎖、零 drift。
//   前提是 DEFAULTS.rho / maxGoals 與 λ 下限(0.15)不可在「未重新鎖定」下改動 —— 改了又不重鎖,
//   玩法機率會相對賽前快照偏移。 改這些常數前先確認重鎖流程。

/**
 * @typedef {Object} SoccerOverUnder
 * @property {number} line     總進球線(.5 → 無和局)
 * @property {number} overPct  大:總進球 > line 的機率(0-100 整數)
 * @property {number} underPct 小:總進球 < line 的機率 = 100 − overPct
 */

/**
 * @typedef {Object} SoccerHandicap
 * @property {number} line     讓球線(.5 → 無和局)
 * @property {number} homePct  主 −line:主隊贏「超過 line 球」的機率(0-100 整數)
 * @property {number} awayPct  客 +line:客隊受讓守住的機率 = 100 − homePct
 */

/**
 * @typedef {Object} SoccerMarkets
 * @property {SoccerOverUnder[]} totals     大小球(每條線一筆)
 * @property {SoccerHandicap[]} handicaps   讓球(每條線一筆)
 * @property {number} bttsYesPct  兩隊進球「是」(雙方都 ≥1 球)的機率(0-100 整數)
 * @property {number} bttsNoPct   兩隊進球「否」= 100 − bttsYesPct
 */

/**
 * 從兩邊預期進球 λ 重建「跟勝/平/負同一張」Dixon-Coles 比分機率表,
 * 推導三個賭徒熟悉的足球玩法機率:大小球 · 讓球(.5 線無和)· 兩隊進球。
 * 純函式、deterministic。 全部用同一份 DEFAULTS(rho / maxGoals)→ 與 1X2 同源、零 drift。
 *
 * 為何 .5 線:無和局(整數比分不會剛好等於 .5)→ 兩邊覆蓋全機率空間 → 互補加總恰 100,
 * 跟莊家「讓 1 球退本金(push)」不同 —— 我們只陳述機率,不模仿賠率結構。
 *
 * @param {number} xgHome  主隊預期進球 λ(取自 prediction.xgHome · 已 ≥0.15)
 * @param {number} xgAway  客隊預期進球 λ(取自 prediction.xgAway · 已 ≥0.15)
 * @param {{ totalLines?: number[], handicapLines?: number[], rho?: number, maxGoals?: number }} [opts]
 * @returns {SoccerMarkets}
 */
export function deriveSoccerMarkets(xgHome, xgAway, opts = {}) {
  const rho = opts.rho ?? DEFAULTS.rho;
  const maxGoals = opts.maxGoals ?? DEFAULTS.maxGoals;
  const totalLines = opts.totalLines ?? [1.5, 2.5, 3.5];
  const handicapLines = opts.handicapLines ?? [1.5, 2.5];
  const lh = Math.max(0.15, xgHome);
  const la = Math.max(0.15, xgAway);

  let total = 0;
  let bttsYes = 0;
  const overByLine = totalLines.map(() => 0);
  const homeCoverByLine = handicapLines.map(() => 0);

  // 同 gridFromLambda 的鋪表順序(x 外、y 內)· 同 τ 修正 → total 與 1X2 那張表逐格一致。
  for (let x = 0; x <= maxGoals; x++) {
    for (let y = 0; y <= maxGoals; y++) {
      const p = poisson(lh, x) * poisson(la, y) * dcTau(x, y, lh, la, rho);
      total += p;
      const sum = x + y;
      for (let i = 0; i < totalLines.length; i++) if (sum > totalLines[i]) overByLine[i] += p;
      const diff = x - y;
      for (let i = 0; i < handicapLines.length; i++) if (diff > handicapLines[i]) homeCoverByLine[i] += p;
      if (x >= 1 && y >= 1) bttsYes += p;
    }
  }

  // 同 gridFromLambda:用整張表的總和正規化(τ 修正會讓總和略偏 1)→ 與 1X2 同一把尺。
  const norm = total > 0 ? total : 1;
  const pct = (/** @type {number} */ v) => Math.round((v / norm) * 100);

  return {
    totals: totalLines.map((line, i) => {
      const overPct = pct(overByLine[i]);
      return { line, overPct, underPct: 100 - overPct };
    }),
    handicaps: handicapLines.map((line, i) => {
      const homePct = pct(homeCoverByLine[i]);
      return { line, homePct, awayPct: 100 - homePct };
    }),
    bttsYesPct: pct(bttsYes),
    bttsNoPct: 100 - pct(bttsYes),
  };
}
