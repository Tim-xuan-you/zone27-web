...
import { TENNIS_DRAW, drawLine, gradeTennisEngine } from "@/lib/tennis/matches";

// ── ZONE 27 · 網球引擎賽前開盤 · 賽後對帳(公開戰績 · 兩向)──────────────────
// 鏡 components/SoccerEngineRecord.tsx,但網球兩向(A 勝 / B 勝 · 無和局)· 排名換算
// 表面校正 Elo 開盤。 賽果 Tim 賽後 curate(同棒球)→ gradeTennisEngine 賽後對帳。
//
// 🔴 誠實鐵律:
//   · 網球熱門贏面天生高(一人控球、磨平運氣)→ 命中率看起來高不是神準,是好預測 → 真尺是校準。
//   · 滿 30 場才報數字(同 CPBL / MLB / 足球)· 幾場的輸贏是運氣不是準度。
//   · 引擎沒開盤的場(剛傷退、排名失真)不進戰績 · 落空照掛、永不刪。
//   · 跟棒球 / 足球分開計(不混池)。
// 純資料(gradeTennisEngine / drawLine 同步純函式)· server component。
// ─────────────────────────────────────────────────────

export default function TennisEngineRecord() {
  const rec = gradeTennisEngine();
    ...
    return [
      {
        ...
        a: m.a.zh,
        b: m.b.zh,
        winnerName: fr.winner === "a" ? m.a.zh : m.b.zh,
        score: fr.score ?? "",
    ...
    <section className="mx-auto max-w-6xl w-full px-6 sm:px-10 pb-12">
      <div className="bg-slate/40 border border-gold/30 p-5 sm:p-7">
        ...
        <p className="text-mute/90 text-sm leading-relaxed mb-5 max-w-2xl">
          網球用<span className="text-bone">排名換算的表面校正 Elo</span> 賽前逐場開盤(改不了)·
          賽後對「當初開的那條線」結算 —— <span className="text-gold">看好邊中沒中、落空照掛、永不刪</span> ·
          跟棒球、足球三本帳分開計(不讓一個運動的雜訊污染另一個)。
        </p>
        ...
        {/* 小樣本誠實:才幾場別讓命中率讀成判決 · 同棒球足球滿 30 場才報 */}
        {!firm && rec.n > 0 && (
        ...
        {/* 網球誠實揭露(永遠顯示 · Pratfall):好預測 ≠ 神準 → 真尺是校準 */}
        <p className="mt-4 text-mute/75 text-[12px] leading-relaxed max-w-2xl">
          <span className="text-bone">為什麼網球看起來會準很多:</span>{" "}
          網球<span className="text-bone">本來就好預測</span>(一個人控制每一球、磨平運氣)——
