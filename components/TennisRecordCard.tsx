...
// ── ZONE 27 · 你的網球戰績(client · 含輸 · 跟棒球 / 足球分開算)──────────────────────
// 讀登入者自己的網球押注(tn-*)· 用賽果 + 引擎當初開盤對帳。 含贏含輸 · 先鎖後結。
// anon / 沒押任何網球 → 不顯示(graceful · 不發空卡)。 賽果還沒 curate → 誠實標「待結算」。
// ─────────────────────────────────────────────────────

export default function TennisRecordCard({
  results,
  enginePicks,
  wrapperClass = "",
}: {
  /** id → { outcome, startISO }(server 傳 · 來自 tennisResults()) */
  results: Record<string, { outcome: TennisPick; startISO: string }>;
  /** id → 引擎看好邊(server 傳 · 來自 tennisEnginePicks())· 給「你 vs 引擎」 */
  enginePicks: Record<string, TennisPick>;
  wrapperClass?: string;
}) {
  const [rec, setRec] = useState<TennisRecord | null>(null);
  const [loaded, setLoaded] = useState(false);

  useEffect(() => {
    let cancelled = false;
    getMyTennisPicks().then((picks) => {
      if (cancelled) return;
      setRec(gradeTennisPicks(picks, results, enginePicks));
      setLoaded(true);
    });
    return () => {
      cancelled = true;
