...
// 讀登入者自己的 MMA 押注(mma-*)· 用賽果 + 引擎當初開盤對帳。 含贏含輸 · 先鎖後結。
// anon / 沒押任何 MMA → 不顯示(graceful)。 賽果還沒 curate → 誠實標「待結算」。
// ─────────────────────────────────────────────────────

export default function MmaRecordCard({
  results,
  enginePicks,
  wrapperClass = "",
}: {
  ...
  const [rec, setRec] = useState<MmaRecord | null>(null);
  const [loaded, setLoaded] = useState(false);

  useEffect(() => {
    let cancelled = false;
    getMyMmaPicks().then((picks) => {
      if (cancelled) return;
      setRec(gradeMmaPicks(picks, results, enginePicks));
      setLoaded(true);
    });
    return () => {
      cancelled = true;
    ...
    <section className={wrapperClass}>
      <div className="bg-slate/40 border border-gold/30 p-5 sm:p-6">
        ...
        <p className="mt-2 font-mono text-mute text-[9px] tracking-[0.12em] leading-snug">
          兩向(誰贏)· 賽前鎖死、賽後對帳 · 跟棒球 / 足球 / 網球 / 羽球準度分開算。
        </p>
      </div>
    </section>
  );
