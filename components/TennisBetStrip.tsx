...
import {
  getMyTennisPrediction,
  getTennisTally,
  submitTennisPrediction,
  setTennisConfidence,
  setTennisRationale,
  crowdPercents,
  TENNIS_CROWD_MIN,
...
// ── ZONE 27 · 網球兩向押注條(A 勝 / B 勝)──────────────────────────────────────
// 押了不可改(先鎖後結)· 登入才能押 · 開賽後鎖手。 純精神預測 = 遊戲 · 0 金額。 賽後自動
// 掛準/不準進你的網球準度(跟棒球 / 足球分開算)。 守暗金:選中上金 · 其餘 mute · 無紅綠。
// 樂觀 UI:點下即翻「已鎖」· server 確認前只說「鎖定中…」(不假裝刪不掉)。
// ─────────────────────────────────────────────────────
...
export default function TennisBetStrip({
  matchId,
  startISO,
  aLabel,
  bLabel,
  ...
  onLock?: (pick: TennisPick) => void;
  /** 顯示「下戰帖給朋友」inline 分享(只今日一戰傳 true)· 預設無 = 不顯示(R294) */
  challenge?: boolean;
}) {
  const [state, setState] = useState<State>("loading");
  const [pick, setPick] = useState<TennisPick | null>(null);
  const [tally, setTally] = useState<TennisTally | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const mine = await getMyTennisPrediction(matchId);
        if (cancelled) return;
        if (mine) {
          setPick(mine);
          setState("picked");
  ...
  const choose = async (p: TennisPick) => {
    if (saving) return;
    ...
    setPending(true);
    setSaving(true);
    const res = await submitTennisPrediction(matchId, p);
    if (res.ok) {
      setPending(false);
      setJustPicked(true);
      onLock?.(p); // 戰帖對決:鎖手確認後揭盅對方那手(預設無 onLock = 行為不變)
      getTennisTally(matchId).then(setTally);
  ...
  const pickLabel = (p: TennisPick) => (p === "a" ? aLabel : bLabel);
    ...
    <div className="mt-3 pt-2.5 border-t border-line/40">
      ...
      {displayState === "picked" && pick && (
        <div className="enter-fade-up border border-gold/40 bg-gold/5 px-3 py-2.5">
          ...
          {!pending && justPicked && (
            <>
              <ConfidencePicker matchId={matchId} submit={setTennisConfidence} />
              <RationalePicker matchId={matchId} submit={setTennisRationale} />
            </>
          )}
...
function CrowdLine({
  ...
  if (tally.total < TENNIS_CROWD_MIN) {
    return (
      <p className="mt-2 font-mono text-mute/45 text-[9px] tracking-[0.15em]">
        {tally.total} 人押了 · 滿 {TENNIS_CROWD_MIN} 人才畫群眾市場線 —— 不拿幾個人假裝是大盤
      </p>
    );
  }
