      ...
      <div className="flex items-stretch gap-1">
        {LEVELS.map((v) => (
          <button
            ...
            disabled={saving}
            onClick={() => choose(v)}
            className="flex-1 min-h-[34px] font-mono text-[10px] tracking-[0.05em] border border-gold/30 text-mute hover:text-gold hover:border-gold/60 hover:bg-gold/5 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          >
            {v / 10}成
          </button>
        ))}
