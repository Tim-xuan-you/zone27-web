...
當戰績 key → 改名/同名再也洗不掉、撞不到戰績。 Tim 親手在 SQL Editor 貼整支 0015 跑完回 Success。
這是創作者天梯 / 「✓已驗證準度」徽章發射前**必修**的命門 —— 現在到位了。
⚠ server 端 RPC 寫入閘門(match_locks 表)仍是待補的 belt-and-suspenders(app 端先鎖後結已防顯示 ·
公開天梯/徽章正式上線前再補)。

### ✅⛔ migration 0013 — 作廢(被 0015 超越 · 切勿單獨重跑)
0013 原本只做一件事:給 `get_creator_records` 加 `created_at`(4 欄版)。 **0015 已把同一支 RPC 重建成
5 欄版(author_code + created_at 都有),完全涵蓋並超越 0013。**
⛔ **0013 不要再單獨貼進 SQL Editor 跑** —— 它會 drop + 重建回舊的 4 欄版,把 0015 的永久碼洗掉(倒退)。
(全新 DB 從 0001 依序套到 0015 沒問題 · 0015 在後面會贏 · 只有「裝完 0015 後再單獨重跑 0013」才有害。)
0013 檔頭已加 SUPERSEDED 警告。 教訓:重跑舊 migration 前,先比對有沒有更新的 migration 重定義同一支函式。
...
### ✅ 兩個發言寫入功能 42702 修復 — 已套(2026-06-05 · R201)

`submit_game_post`(賽事討論室)+ `submit_creator_comment`(留言)的 `returning id, created_at` 撞
RETURNS TABLE 的 out 欄 created_at → **42702 ambiguous** → 發言其實送不出(前端 graceful catch 成假
訊息 = R181 同陷阱)。 repo 早已 qualify 表名修好。 Tim 2026-06-05 在 SQL Editor 套上**修正版的兩個
寫入函式那一塊**(不是整支檔)· 跑完 Success → 留言 + 討論室發言真的通了。
⛔ **重要修正(取代舊指示「重貼整支 0004 + 0010」=== 那是地雷):0010 整支絕不可在 0015 之後重跑** ——
0010 檔裡有舊的 `get_creator_comments`(4 欄),整支重跑會把 0015 升級的 6 欄版打回去(倒退/或 42P13)。
正解 = 只套「submit_creator_comment + submit_game_post 兩個寫入函式」那塊(已套)。 0004 整支因不碰
creator_* 讀取函式 · 重跑無害(但不必)。 0010 檔頭已加警告。
...
- ✅✅✅ **R200(2026-06-04 · 新窗省 token · 接 R199 · 全三綠 + 13/13 單元驗 rename 不變量 + 實機 SSR + auto-push)· Tim 全權「攻頂 · 先把顯示名問責命門補掉」**:
  - 🔴🔴 **顯示名問責命門 = DONE(本窗第一件 · 比美觀重要 · commit `966e63e`)**:修「創作者改名洗掉戰績」架構命門 —— 0014 讓戰績 key 綁可變的 `handle`(顯示名)→ 改名/同名洗掉、撞掉戰績 = 報馬仔刪輸文的反面。 **正解**(全平台標準 X/Discord/Reddit/Polymarket):戰績綁**永久碼** `author_code`(md5(uid) 前 8 · 改名洗不掉、同名不撞)· 永久碼永遠顯示在名字旁(`大師 · #46f6741a`)· 改名免費無限次。 落地:`migration 0015`(三支 RPC posts/comments/records 各多回 `author_code` + `display_name` · 先 drop 避 42P13 · 全 qualify 避 42702 · **Tim 待套** 見上方 🔴)· `lib/identity.ts` 新 `creatorIdentity` + `creatorRecordKey`(單一真相 · graceful fallback 舊 RPC 回 handle 不破)· `gradeAuthorRecords` key 改永久碼 · `CreatorAnalysis`(分析署名 + 留言者)+ `DisplayNameSetting`(/member 自己的臉/碼)全 reuse `creatorIdentity` = 公開署名跟後台同一張臉、頭像色綁永久碼改名不換。 文案「取代代號」→「名字只是標籤 · 永久碼扛問責 · 改名免費」。 **驗證**:13/13 單元(改名 key 不變 / 同名不撞 / 色穩定 / fallback / 格式 / posts↔records 對得上)against 真實編譯的 identity.ts · 三綠 + 5 路由 SSR 200 + reverse-grep 確認 fresh build。
  - 🧹 **R199 砍頁善後 = DONE(commit `221a072` · Tim 拍板「維持全砍 · 改寫文案」)**:R199 砍 6 內部頁變 redirect,但 ~50 處連結/文案仍指向它們(尤其「規則修改 30 天前公告於 /changelog」散落 31 處 + /roadmap→/changelog→/roadmap 繞圈)。 發現 /annual + /founders/postmortem-2028 曾列「永遠不刪信任頁」被誤砍 → **問 Tim · Tim 選維持全砍**。 重新指向倖存頁:`/changelog`→公開 GitHub commit 史(本就是站上唯一事實來源 · 同 /about/Footer 慣例 · 修繞圈)· `/annual`→保留承諾改「年度報告」/透明錨點走 /audit · `/founders/postmortem-2028`→/steelman · `/hey-tim`→/faq+/member/submit(BLACK perk OG 對齊「寫信給 Tim 24h 回」)· `/now`→/roadmap · /interact 砍「Hey Tim 公開問答」死管道(6→5 renumber)· 刪孤兒 `UnscheduledLetterChip` + `lib/hey-tim-entries`。 三綠 + 16 路由 SSR 200 + 6 已刪頁仍 307 + reverse-grep served HTML 全站 0 dead href · net −294 行。
  - 🎯 **craft O2 = DONE(commit `ffcd3e5`)**:詳情頁「選邊兩次」困惑 → 已押者發分析時推薦邊預填成押的那邊(getMyPrediction · 一鍵可改 · 標「✓ 你押的那邊」)· 沒押的維持原說明。
  - 🏛️🏅 **三層樓 IA 決策 + 個人榮譽牆 = DONE(commit `4c53b81`)· Tim 連看 3 張玩運彩圖(徽章/追蹤明燈/「像 Apple 怎麼規劃」)+「您決定」**:專家 reframe「玩運彩加法 vs Apple 減法」→ 定三層樓(看/對帳/地位 · 徽章追蹤=三樓配角永不上首頁)。 落地第一塊 `components/HonorWall.tsx`(/member 校準卡下方 · 5 章全從含輸帳本自動算 · 報馬仔抄不走 · 0 新 migration · 多情境截圖驗)。 詳見 [[project-zone27-soul-roadmap]] 三層樓段 + 追蹤/明燈裁決。
  - 🌧️⚾ **延賽誠實處理 + 06/05 三場 ingest(commit `87ed61b` / `f007077`)**:#141 延賽 → Match.postponed + getMatchPhase 視同 stale-archived(自動下板 · 詳情頁押注關 + 顯示延賽 · 開盤線保留)· 06/05 三場真投手數據算引擎線(每場推理寫進註解 · validate:data 0 error)+ 補洲際球場。
  - 🚀 **全權攻頂 · 3-agent synthesize → 3 波 ship(Tim AFK「自己迭代/查全世界/認真看bug/熱銷/只做最好」)**:3 路平行 agent(碼審 / 全球研究缺的靈魂 / 轉換美觀)· **碼審結論再確認:0 P0/P1 · 護城河紮實**(先鎖後結/wallet 原子/admin gate/0015 永久碼/sanitizeNext 全硬)。 ship:
    - **Wave 1(`1433b33`)**:/matches 去工程黑話(`ingest`/`per /coverage`/`receipt ledger`→白話)· YourRecordStrip getSession→getUser(補唯一漏網 cookie-only gate)· /member 升級入口『賣分析賺錢』→『把準度變成身分』· /matches MLB 卡 dedup。
    - **Wave 2(`dda930f`)= 唯一真整合洞**:`getMlbFinalizedResults` 原靠即時 API 2 天窗 → settled MLB 押注 2 天後變回 pending(違反刪不掉帳本)· 改讀 `mlb-locked.json` 永久 finalScore(14 場)+ live 只補剛打完 · 對齊 CPBL 永久戰績。
    - **Wave 3(`c03bf63`)= C5**:/membership H1『解鎖賣分析賺錢』→『把你的準度變成身分』· 賺錢降括號副利 · BLACK kicker 進場/GOLD 最高階對仗(paid=身分非功能)。
    - 🌍 **全球研究驗證方向**(Polymarket/Manifold/Metaculus/Pinnacle/Baseball Savant):我們的校準身分/含輸戰績/永久碼/reasoning 卡 = 世界級對的方向(多數已 ship)· 唯一新點子待評:Pinnacle「Winners Welcome」式**明說的「輸的永不刪 · 包括創作者跟站方」costly-signal 政策**(no-DB · 但恐與 /integrity 重疊 · 待查再加避免冗餘)。
  - **下窗 backlog**:soul ②streak ③含輸分享卡(待 Tim social go)⑤留言「已下注」徽章+公開 profile · ⑥校準榜+追蹤(都需 migration·用戶多才發光·**設計已鎖** 見 soul-roadmap)· craft **C4**(/matches 板露創作者「✓大神 78%」· 需 server 端 creator-records+grade 路徑 · M-effort · 0 創作者時 graceful 不顯示)· 哲學長文 consolidate 進 /about(⚠️ Tim sacred · 別盲砍)· MLB 真隊色微調 · ⏳ **Tim 仍待套 migration 0015**(顯示名永久碼公開生效)。
