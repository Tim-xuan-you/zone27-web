...
- ✅👤 **R193 創作者後台 v1(2026-06-05 · Tim GOLD dogfood「會員介面看不到我發了哪些文章/幾勝幾敗/有人回嗎/怎麼聯絡站長」)**:
  ...
  - ⏳ **沒答完 · 待下一支 RPC(乾淨解)**:#6「有哪些人買我的文」+ #7 全站彙整。 v1 用 getMyCreatorPost 逐場撈(該 RPC 不回 post_id → 撈不到 buyer/aggregated reply · N 次呼叫規模化會慢)。 **新 migration `get_my_creator_posts()` SECURITY DEFINER**:一次回我的文(post_id/match_id/title/price/pick/created_at)+ 各篇 `reply_count`(join creator_comments)+ `buyer_count`(join 點數購買紀錄)· ⚠ buyer 名單露不露(隱私)Tim 定 · 先給「N 人買」數字即可。
